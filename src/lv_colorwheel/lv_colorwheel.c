 ...
 *  STATIC PROTOTYPES
 **********************/
static lv_design_res_t lv_colorwheel_design(lv_obj_t * colorwheel, const lv_area_t * clip_area, lv_design_mode_t mode);
...
lv_obj_t * lv_colorwheel_create(lv_obj_t * parent, bool knob_color)
{
    ...
    if(ancestor_design == NULL) ancestor_design = lv_obj_get_design_cb(colorwheel);
    ...
    lv_obj_set_design_cb(colorwheel, lv_colorwheel_design);
...
static lv_area_t get_knob_area(lv_obj_t * colorwheel)
{
    ...
    lv_style_int_t left = lv_obj_get_style_pad_left(colorwheel, LV_COLORWHEEL_PART_KNOB);
    lv_style_int_t right = lv_obj_get_style_pad_right(colorwheel, LV_COLORWHEEL_PART_KNOB);
    lv_style_int_t top = lv_obj_get_style_pad_top(colorwheel, LV_COLORWHEEL_PART_KNOB);
...
static lv_res_t lv_colorwheel_signal(lv_obj_t * colorwheel, lv_signal_t sign, void * param)
{
    ...
    if(sign == LV_SIGNAL_CLEANUP) {
        ...
        if((LV_MATH_ABS(p.x - ext->last_press_point.x) > DRAG_LIMIT) ||
           (LV_MATH_ABS(p.y - ext->last_press_point.y) > DRAG_LIMIT)) {
            ext->last_change_time = lv_tick_get();
        ...
        int16_t angle = 0;
        lv_style_int_t cir_w = lv_obj_get_style_scale_width(colorwheel, LV_COLORWHEEL_PART_MAIN);
...
static void refr_knob_pos(lv_obj_t * colorwheel)
{
    ...
    lv_style_int_t scale_w = lv_obj_get_style_scale_width(colorwheel, LV_COLORWHEEL_PART_MAIN);
